...
The executable feature is defined in `bdd/features/readiness.feature` with three scenarios:
happy path (Transition Lead submits complete assessment), missing evidence (submission blocked),
and unauthorized user (Contributor cannot submit). See that file for the Gherkin source.
